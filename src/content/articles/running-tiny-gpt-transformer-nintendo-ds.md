...
That is useful for a first baseline because the DS runtime and the PyTorch
reference are doing the same simple job at each generation step: take the
visible tokens, run the full context, and produce logits for the next token.
There is no KV cache, rolling state, or cache eviction policy to debug yet, so a
mismatch has fewer places to hide. It is also painful on original DS hardware
because every generated byte repeats work for earlier visible bytes.
