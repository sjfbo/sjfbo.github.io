...
This is the first article in a series, so it starts with the baseline pass: the
slow, direct version that is easiest to trust. The job here is to train a tiny
byte-level decoder on a host machine, export a narrow model file, validate the C
runner against PyTorch, and run the same weights on the DS. Later parts can
optimize the runtime only because this plain version is legible first.
...
generated byte. That is wasteful, but it keeps the native runner and the DS
build on the same simple execution path.
...
That is useful for a first baseline because the native C runner and the DS ROM
are doing the same simple job at each generation step: take the visible tokens,
run the full context, and produce logits for the next token. There is no KV
cache, rolling state, or cache eviction policy to debug yet, so a mismatch has
fewer places to hide. It is also painful on original DS hardware because every
generated byte repeats work for earlier visible bytes.
...
That is why the next parts of the series move in two directions at once:
...
about every model shape. They are the reason this article stops at the simple
baseline and leaves the faster runtime work for the follow-up parts.
...
The baseline is deliberately plain: it is slow, simple, and easy to compare
against. As the first part of the series, that is the point. The next parts can
then turn "a Transformer can run" into "a Transformer can run fast enough to feel
