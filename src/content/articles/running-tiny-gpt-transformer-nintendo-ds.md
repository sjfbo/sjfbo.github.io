...
That is useful for a first baseline because it is easy to compare against the
PyTorch reference: same visible tokens in, same full-context forward pass, same
logits out. It is also painful on original DS hardware because every generated
byte repeats work for earlier visible bytes.
